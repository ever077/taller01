...
public class BibliotecaList implements Biblioteca{
	...
	private MaterialCapacitacion buscadorBinario(int i, int f,  Integer costo) {
		// TODO Auto-generated method stub
		int tam=(i+f)/2;
		if((materiales.get(tam).getCosto().intValue()) == costo)
		{
			return materiales.get(tam);
		}
		if((materiales.get(tam).getCosto().intValue()) < costo) {
						
			buscadorBinario(tam+1, f, costo);
		}
		if((materiales.get(tam).getCosto().intValue()) > costo)
		{
			buscadorBinario(i, tam, costo);
		}
		else {
		 RuntimeException("Material de precio "+costo+" no encontrado"); 
